    ...
    public class HubEndPoint<THub, TClient> : EndPoint, IInvocationBinder where THub : Hub<TClient>
    {
        ...
        public override async Task OnConnectedAsync(ConnectionContext connection)
        {
            try
            {
                ...
                var hubProtocolFeature = new HubProtocolFeature();
                hubProtocolFeature.HubProtocol = _protocolResolver.GetProtocol(connection);
                connection.Features.Set<IHubProtocolFeature>(hubProtocolFeature);
        ...
        private async Task DispatchMessagesAsync(ConnectionContext connection)
        {
            ...
            try
            {
                while (await channel.Input.WaitToReadAsync(cts.Token))
                {
                    while (channel.Input.TryRead(out var incomingMessage))
                    {
